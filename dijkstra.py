...
N = len(G) #кол-во элементов в словаре (G)

t = [] #список посещенных вершин
p = {} #словарь {открытая вершина : ее метка}
...
def dijkstra(v, p, t, b, e):
    ...
    for x in G[v]: #для каждого соседа (x) текущей вершины (v)
        ...
        if not x in p:  #если соседа (x) нет в словаре (p)
            ...
            b[x] = v    #как только метка пересчитывается, запоминаем
                        #(следующая вершина: предудущая вершина) в словаре (b)
            ...
            if p[x] > xm: #если старая метка соседа больше новой метки
                ...
                b[x] #как только метка пересчитывается, запоминаем
                    #(следующая вершина: предыдущая вершина) в словаре (b)
        
        print('текущей вершины v = ', v, ' сосед x = ', x, ' с меткой xm = ', xm)
    ...
    if N <= len(t): #Условие выхода и функции
        print('\nВсе!\nВершины и их метки = ', p)
        print('Словарь для отслеживания пути = ', b)

        s = [] #кратчайший путь
        s.insert(0, e) #вставляем (e) в список (s) по индексу (0)
